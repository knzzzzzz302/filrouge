...
import { z } from 'zod';
const router = Router();
function estimatedPriceBase(city, postalCode) {
...
router.get('/search', async (req, res) => {
    ...
    try {
        const communeUrl = `https://geo.api.gouv.fr/communes?nom=${encodeURIComponent(query)}&fields=nom,code,codesPostaux,population,centre&boost=population&limit=6`;
        ...
        const addressesPayload = (await addressRes.json());
...
router.get('/images', async (req, res) => {
    ...
    try {
        const url = `https://api.openverse.org/v1/images/?q=${encodeURIComponent(`real estate ${query} house apartment`)}&license_type=commercial,modification&filter_dead=true&page_size=12`;
...
router.post('/estimate', async (req, res) => {
    ...
    const conditionFactor = data.condition === 'EXCELLENT' ? 1.08 : data.condition === 'GOOD' ? 1 : 0.87;
    const energyFactor = data.energyClass === 'A'
        ? 1.08
        : data.energyClass === 'B'
            ? 1.05
            : data.energyClass === 'C'
    ...
    const estimateRaw = data.areaM2 * basePerM2 * locationFactor * typeFactor * conditionFactor * energyFactor * distanceFactor +
        roomsBoost;
