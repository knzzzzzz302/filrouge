...
router.post('/', requireAuth, requireRole(['COMMERCIAL', 'AGENCY_MANAGER', 'ADMIN']), async (req, res) => {
    ...
    try {
        const created = await pool.query('INSERT INTO transactions(property_id, amount, status) VALUES ($1,$2,$3) RETURNING id, property_id, amount, status', [property_id, amount, status]);
...
router.patch('/:id/status', requireAuth, requireRole(['COMMERCIAL', 'AGENCY_MANAGER', 'ADMIN']), async (req, res) => {
    ...
    try {
        const updated = await pool.query('UPDATE transactions SET status = $1 WHERE id = $2 RETURNING id, property_id, amount, status', [status.data, req.params.id]);
        if (!updated.rows[0])
