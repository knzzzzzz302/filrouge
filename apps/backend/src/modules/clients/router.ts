...
const fallbackClients: Array<{ id: number; first_name: string; last_name: string; email: string; budget_max: number }> = [
...
router.get('/', requireAuth, async (_req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, first_name, last_name, email, budget_max FROM clients ORDER BY id DESC LIMIT 100'
    );
...
router.post('/', requireAuth, requireRole(['COMMERCIAL', 'AGENCY_MANAGER', 'ADMIN']), async (req, res) => {
  ...
  try {
    const created = await pool.query(
      'INSERT INTO clients(first_name, last_name, email, budget_max) VALUES ($1,$2,$3,$4) RETURNING id, first_name, last_name, email, budget_max',
      [body.data.first_name, body.data.last_name, body.data.email, body.data.budget_max]
    );
